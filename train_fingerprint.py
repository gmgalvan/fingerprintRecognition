...
print(os.getcwd())
#%% Load Dataset
x_real = np.load('dataset/X_imgs.npz')['arr_0']
x_real = x_real[:, :, :, np.newaxis]
y_real = np.load('dataset/y_labels.npy')

X_augs1 = np.load('dataset/X_augs1.npz')['arr_0']
X_augs1 = X_augs1[:, :, :, np.newaxis]
#y_easy = np.load('dataset/y_easy.npy')

X_augs2 = np.load('dataset/X_augs2.npz')['arr_0']
X_augs2 = X_augs2[:, :, :, np.newaxis]
#y_medium = np.load('dataset/y_medium.npy')

X_augs3 = np.load('dataset/X_augs3.npz')['arr_0']
X_augs3 = X_augs3[:, :, :, np.newaxis]
...
plt.subplot(1, 4, 2)
plt.title(y_real[i])
plt.imshow(X_augs1[i].squeeze(), cmap='gray')
plt.subplot(1, 4, 3)
plt.title(y_real[i])
plt.imshow(X_augs2[i].squeeze(), cmap='gray')
plt.subplot(1, 4, 4)
plt.title(y_real[i])
plt.imshow(X_augs3[i].squeeze(), cmap='gray')
#%% Train Test Split
x_data = np.concatenate([X_augs1, X_augs2, X_augs3], axis=0)
label_data = np.concatenate([y_real, y_real, y_real], axis=0)
...
plt.subplot(2, 5, 1)
plt.title('original')
plt.imshow(x_data[1000].squeeze(), cmap='gray')
...
#hace un recorrido adentro de y_real y las 4 etiquetas las pasa a formato cadena y las junta todas en una sola,
#y las guarda en un diccionario con su propio indice
...
class DataGenerator(keras.utils.Sequence):
    ...
    def __getitem__(self, index):
        ...
        # augmentation
        # realiza por cada huella genera una copia modificada
        if self.shuffle:
            seq = iaa.Sequential([
                iaa.GaussianBlur(sigma=(0, 0.5)),
                iaa.Affine(
...
model.summary()
#%% Train
history = model.fit_generator(train_gen, epochs=15, validation_data=val_gen, use_multiprocessing=True)

model.summary()
model.save('modelo_db')
model = keras.models.load_model('modelo_db')
#%% Evaluation
